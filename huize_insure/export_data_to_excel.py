...
user = 'root'
passwd = 'root'
db_name = 'xxxx'  # TODO 目标数据库
table_name = 'insure_product_info2'  # TODO 目标表
...
def export_data_to_excel():
    schema = Info_schema()
    # excel 文件名
    excel_name = "D:/PyProject/scripts/huize_insure/data/insures"
...
class Info_schema():
    def __init__(self):
        self.conn = pymysql.connect(user=user, passwd=passwd, host=host, db='information_schema', charset='utf8')
        self.cursor = self.conn.cursor()

    # 查询表注释
    def get_table_name(self):
        sql_query_comment = "SELECT table_comment FROM TABLES WHERE table_schema = '%s' AND table_name = '%s' GROUP BY table_name; " % (
        db_name, table_name)
    ...
    def get_column_comments(self):
        sql_query_columns = "SELECT column_name, column_comment, column_type FROM COLUMNS WHERE table_schema = '%s' AND table_name = '%s';" % (
        db_name, table_name)
...
class DB():
    def __init__(self):
        self.conn = pymysql.connect(user=user, passwd=passwd, host=host, db=db_name, charset='utf8')
        self.cursor = self.conn.cursor()
